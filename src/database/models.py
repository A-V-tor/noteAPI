...
from sqlalchemy import (Text, Boolean, DateTime, Integer, String, ForeignKey,
                        Table, func, Column, select, and_, desc, delete)
...
class User(Base):
    ...
    async def add_user(cls, username: str, password: str):
        ...
        async with async_session_factory() as session:
            ...
            token = await cls.generate_jwt_token(new_user.id)
    ...
    async def get_user(cls, username: str, password: str):
        async with async_session_factory() as session:
            user = await session.scalar(select(cls).where(and_(cls.username==username)))
    ...
    @classmethod
    async def update_jwt_token(cls, username: str, password: str):
        async with async_session_factory() as session:
            user = await session.scalar(select(cls).where(and_(cls.username == username)))
            check_password = pwd_context.verify(password, user.password)

            if user and check_password:
                token = await cls.generate_jwt_token(username)
                user.token = token
                await session.commit()
    ...
    async def generate_jwt_token(username: str):
        ...
        token = jwt.encode(payload, settings.secret_key, algorithm=settings.algorithm_hash)
...
class Note(Base):
    ...
    async def get_all_notes(cls, user_id: int):
        async with async_session_factory() as session:
            notes = (await session.execute(select(cls).where(cls.user_id.in_([user_id])).order_by(desc(cls.created_at)))).scalars().all()
            return notes

    @classmethod
    async def create_note(cls, user_id: int, title: str, text: str, tags: list['Tag']):
        async with async_session_factory() as session:
    ...
    async def get_note(cls, id_: int):
        async with async_session_factory() as session:
            note = await session.scalar(select(cls).where(cls.id.in_([id_])).options(selectinload(cls.tags)))
            return note
...
class Tag(Base):
    ...
    async def get_tags_by_title(cls, title_list: list):
        async with async_session_factory() as session:
            tags = (await session.execute(select(cls).where(cls.title.in_(title_list)))).scalars().all()
            if not tags:
